        ...
        if contrast == 1:  # SNS changing this for testing!
            ...
            if slice == nslices - 1:  # if last slice & second echo-time
                ...
                if args.running:
                    ...
                    # im_path = ("/home/sn21/miniconda3/envs/gadgetron/share/gadgetron/python/results/2023-09-26/10-02"
                    #            "-16-gadgetron-fetal-brain-localisation-mask_img-0.nii.gz")
                    # im = nib.load(im_path)
                    # im = im.get_fdata()
                    threshold = 17.5  # Adjust this threshold as needed
                    binary_mask = (im > threshold).astype(np.uint8)
                    ...
                    xcm, ycm, zcm = ndimage.center_of_mass(binary_mask)
                    # xcm = model.x_cm
                    # ycm = model.y_cm
                    # zcm = model.z_cm
                    ...
                    if repetition == 0:
                        ...
                        segmentation_volume = binary_mask
                        image_volume = im
                        ...
                        full_path_seg = os.path.join(box_path, timestamp, "nnUNet_seg")
                        full_path_pred = os.path.join(box_path, timestamp, "nnUNet_pred")

                        # Create the necessary directories, including any missing parent directories
                        os.makedirs(full_path_seg, exist_ok=True)
                        os.makedirs(full_path_pred, exist_ok=True)

                        bin_im = nib.Nifti1Image(binary_mask, np.eye(4))
                        nib.save(bin_im, full_path_pred + "/FreemaxLandmark_001_0000.nii.gz")
                        ...
                        xcm = (pixdim_x * np.int(im.shape[0])) / 2
                        ycm = (pixdim_y * np.int(im.shape[1])) / 2
                        zcm = (pixdim_z * np.int(im.shape[2])) / 2

                        # xcm = np.int(im.shape[0] / 2 + 1)
                        # ycm = np.int(im.shape[1] / 2 + 1)
                        # zcm = np.int(im.shape[2] / 2 + 1)

                        # This is for a perfect sagittal
                        cm_brain = xcm, ycm, zcm-10
                        cm_eye_1 = xcm+60, ycm+30, zcm  # 30,0,60
                        cm_eye_2 = xcm+60, ycm-30, zcm  # -30,0,60
                        cm_cereb = xcm-60, ycm, zcm-20
                        furthest_point = xcm+65, ycm, zcm+10

                        # # This is for a perfect coronal
                        # cm_brain = xcm, ycm, zcm
                        # cm_eye_1 = xcm+60, ycm, zcm+30  # 0,30,60
                        # cm_eye_2 = xcm+60, ycm, zcm-30  # 0,-30,60
                        # cm_cereb = xcm-60, ycm, zcm
                        # #
                        # # This is for a perfect transverse
                        # cm_brain = xcm, ycm, zcm
                        # cm_eye_1 = xcm+30, ycm, zcm+60  # 0,60,30
                        # cm_eye_2 = xcm-30, ycm, zcm+60  # 0,60,-30
                        # cm_cereb = xcm, ycm, zcm-60
                        ...
                        centreofimageposition = ((np.float(enc.encodedSpace.fieldOfView_mm.x)/2,
                                                  np.float(enc.encodedSpace.fieldOfView_mm.y)/2,
                                                  np.float(nslices*pixdim_z) / 2))
